        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <form action="/admin/alerts" method="get" class="flex gap-2 items-center">
            ...
            <%= if @filters.search && @filters.search != "" do %>
              <a href={remove_filter_url(@filters, :search)}
          ...
          <div class="mt-3 flex flex-wrap gap-2 items-center">
            ...
            <%= for {key, label, value} <- active_filters(@filters) do %>
              <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs
                ...
                <a href={remove_filter_url(@filters, key)}
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <%= if @alerts == [] do %>
          <div class="px-6 py-16 text-center">
            ...
            <%= if active_filters(@filters) != [] or (@filters.search && @filters.search != "") do %>
              <a href="/admin/alerts" class="text-sm text-indigo-500 hover:underline mt-2 inline-block">
                Clear all filters
              </a>
            <% end %>
          </div>
        <% else %>
              ...
              <tbody class="divide-y divide-gray-100">
                <%= for alert <- @alerts do %>
                  <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                      <a href={"/admin/alerts?user_id=#{alert.user_id}"}
                         class={[
                    ...
                    <td class="px-6 py-4">
                      <a href={"/admin/alerts?shelly_id=#{alert.shelly_id}"}
                         class={[
                           "hover:underline",
                    ...
                    <td class="px-6 py-4 text-right">
                      <div class="flex justify-end gap-2">
                        ...
                        <a href={"/admin/alerts?user_id=#{alert.user_id}&shelly_id=#{alert.shelly_id}"}
          ...
          <.pagination page={@page} total_pages={@total_pages} base_path={@base_path}
                       total_count={@filtered_count} per_page={@per_page} />
        <% end %>
      </div>
    </div>
    """
